...
export function compile(
    ...
    for (const rule of styleRules) {
        for (const selectorComponents of rule.selectors) {
            ...
            const { properties, pseudoInstances, overflowScroll } =
                mapDeclarations(rule.declarations, warnings);
...
function generateBaseElementRules(irRules: StyleRuleIR[]): void {
    ...
    const transparentBg: RobloxValue = { type: "number", value: 0 };
    ...
    // TextBox: auto-size to text content, transparent background (like <input>)
    const textBoxProps = new Map<string, RobloxValue>();
    textBoxProps.set("AutomaticSize", autoSizeXY);
    textBoxProps.set("BackgroundTransparency", transparentBg);
    irRules.unshift({
        selector: "TextBox",
    ...
    // Frame: transparent background by default (like <div> - no visible background)
...
function generateSizeCompoundRules(irRules: StyleRuleIR[]): void {
    ...
    for (const rule of irRules) {
        ...
        const autoStr =
            autoVal && autoVal.type === "Enum" ? autoVal.value : null;
